...
@section('title',$user->full_name.' Info')

@section('content') 

<main class="user-info-wrapper col-md-9 ms-sm-auto col-lg-10 px-md-4 pt-5 position">
@include('notification.notify')
  <h2 class="table-cap pb-2 mb-3 text-capitalize">{{$user->full_name}} <span class="user-info-id">AF{{$user->id}}</span></h2>
...
<a href="javascript:void(0)" data-bs-toggle="modal" data-bs-target="#remove-item" data-id="927" data-url="{{route('admin.deleteuserprofilephoto',[$user->id])}}" class="deleteitem cross-img" tabindex="0">X</a>
        <img src="{{$user->profile}}" alt="User Info Profile Pic">
      ...
      <div class=" table-responsive tenant-table ">
        <caption>
          ...
          @if(!empty($user->contract))
           <a href="" type="submit" onclick="window.open('{{asset('uploads/').'/'.$user->contract}}')" class="contract-btn float-end mb-2">View Contract</a>
          @endif
        ...
        <table class="table  table-bordered">
          ...
          <tbody>
            <tr>
              ...
              <td>{{@$user->property}} & {{@$user->apt_number}}</td>
              <td>{{$user->tenant_type}}</td>
              <td>{{$user->type}}</td>
              <td>{{@$user->vehicles[0]['name']}}</td>
            </tr>       
          </tbody>
        </table>
      </div>
    ...
    <div class="col-xxl-6 col-xl-6">
      <div class=" table-responsive tenant-table unique-table">
        ...
        <table class="table  table-bordered" id="housekeeper">
          ...
          <tbody>
            @foreach($user->housekeeping_requests as $housekeeper)
            <tr>
    ...
    <div class="col-xxl-6 col-xl-6">
      <div class=" table-responsive tenant-table">
        ...
        <table class="table  table-bordered" id="lifestyle">
          ...
          <tbody>
            @foreach($user->lifestyleview() as $lifestyle)
            <tr>
    ...
    <div class="col-xxl-6 col-xl-6">
      <div class=" table-responsive tenant-table">
        ...
        <table class="table  table-bordered " id="concierge">
          ...
          <tbody>
            @foreach($user->conciergeview()  as $concierge)
