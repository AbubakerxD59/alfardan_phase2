...
@section('title','Concierge Chat')

@section('content')
<main class="event-wrapper chat col-md-9 ms-sm-auto col-lg-10 px-md-4 position">
<div class="row">
<div class="col-md-4 col-lg-4 col-sm-12 service-chat">
	<div class="search-holder">
		<h2 class="table-cap chat-heading">Concierge Chat</h2>
	...
	<div class="customscroll user_data">
		...
		{{--
		<?php
		$message=$chat->messages()->orderby('id','desc')->groupBy('chat_id')->get();
		
		?>
		@foreach($message as $value)

		<div class="customer-side customer-top user-chat cursor-pointer" id="{{$chat->id}}">
			<div class="customer-info">
				<div class="chat-img ">
					<!-- <i class="fa fa-user-circle"></i> -->
					
					@if(!empty($value->sender->profile))
					<img src="{{@$value->sender->profile}}">
					@else
					<img src="{{asset('alfardan/assets/profile-pic.jpg')}}" alt="...">
					@endif
				</div>
				<div class="customer-detail">
					<h4>{{@$value->sender->full_name}}</h4>
					<p>12{{@$value->text}}</p>
				</div>

			</div>
			<div class="service-info">
				<h4>{{@$value->sender->type}}</h4>
...
<div class="col-md-8 col-lg-8 col-sm-12 chat-profile-holder" id="chat-profile-holder">
...
<script>
	...
	function search(){
	     ...
	     $.post('{{ route("admin.conciergechatSearch") }}',
	      {
